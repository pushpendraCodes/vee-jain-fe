...
import { ApiError, fetchCities, fetchMe, fetchMyCredit, fetchStates, updateAvatar, updateProfile, type CreditAccountPayload } from "@/lib/api";
...
function DetailRow({ label, value }: { label: string; value?: string | null }) {
  return (
    <div className="grid grid-cols-[7.5rem_1fr] gap-4 border-t border-line py-3.5 first:border-t-0">
...
export default function ProfilePageClient() {
  ...
  const [credit, setCredit] = useState<CreditAccountPayload | null>(null);
        ...
        <div className="mx-auto flex max-w-5xl flex-col gap-6 px-5 py-10 sm:flex-row sm:items-end sm:justify-between sm:px-6 lg:px-8 lg:py-12">
          <div className="flex items-center gap-5">
            <div className="relative h-[88px] w-[88px] shrink-0">
              <div className="flex h-[88px] w-[88px] items-center justify-center overflow-hidden rounded-full border-2 border-line bg-surface-2 text-2xl font-semibold text-ink-dim">
                {user.avatar ? (
              ...
              <button
                ...
                disabled={uploading}
                className="absolute bottom-0 right-0 flex h-8 w-8 items-center justify-center rounded-full border-2 border-surface bg-brand text-accent-on disabled:opacity-70"
            ...
            <div>
              <p className="text-[11px] font-semibold uppercase tracking-[0.14em] text-ink-dim">Account</p>
          ...
          {!editing ? (
            <button
              type="button"
              onClick={startEdit}
              className="inline-flex h-11 items-center gap-2 self-start rounded-full bg-surface px-5 text-sm font-semibold text-accent-on transition hover:bg-surface-2 sm:self-auto"
            >
        ...
        {editing ? (
          <form onSubmit={onSave} className="space-y-5" noValidate>
            <section className="rounded-2xl bg-surface p-6 sm:p-8">
              <p className="text-[11px] font-bold uppercase tracking-[0.08em] text-ink-dim">Personal details</p>
            ...
            <section className="rounded-2xl bg-surface p-6 sm:p-8">
              <p className="text-[11px] font-bold uppercase tracking-[0.08em] text-ink-dim">Company details</p>
              <div className="mt-5 grid gap-4 sm:grid-cols-2">
                <div>
            ...
            <section className="rounded-2xl bg-surface p-6 sm:p-8">
              <p className="text-[11px] font-bold uppercase tracking-[0.08em] text-ink-dim">Address</p>
            ...
            <div className="flex flex-wrap gap-3">
              <button type="button" onClick={cancelEdit} className="h-11 rounded-full border border-line bg-surface px-6 text-sm font-medium text-text-primary">
                Cancel
              </button>
              <button type="submit" disabled={saving} className="h-11 rounded-full bg-surface-2 px-7 text-sm font-semibold text-on-dark disabled:opacity-60">
          ...
          <div className="space-y-5">
            ...
            {credit?.account ? (
              <section className="rounded-2xl bg-surface p-6 sm:p-8">
                <p className="text-[11px] font-bold uppercase tracking-[0.08em] text-ink-dim">Business credit</p>
                <dl className="mt-2">
                  <DetailRow label="Account" value={credit.account.accountNumber} />
                  <DetailRow label="Status" value={credit.account.status} />
                  <DetailRow label="Limit" value={formatINR(credit.account.creditLimit)} />
                  <DetailRow label="Used" value={formatINR(credit.account.balance)} />
                  <DetailRow label="Available" value={formatINR(credit.account.available)} />
                  <DetailRow label="Credit days" value={String(credit.account.creditDays)} />
                </dl>
                <div className="mt-3 h-2 overflow-hidden rounded-full bg-bg">
                  <div className="h-full rounded-full bg-surface-2" style={{ width: `${Math.min(100, credit.account.utilization)}%` }} />
                </div>
                {credit.account.transactions?.length ? (
                  <div className="mt-5">
                    <p className="text-[11px] font-bold uppercase tracking-[0.08em] text-text-secondary">Statement</p>
                    <ul className="mt-2 divide-y divide-border-hairline">
                      {credit.account.transactions.slice(0, 8).map((t) => (
                        <li key={t.id || `${t.createdAt}-${t.amount}`} className="flex items-center justify-between py-2 text-sm">
                          <span className="text-text-secondary">{t.description || t.type}</span>
                          <span className="font-medium text-text-primary">{formatINR(t.amount)}</span>
                        </li>
                      ))}
                    </ul>
                  </div>
